...
by explicitly giving the correlation that achieves self-testin. 
...
some unknown operator which is decomposed into two binary observables.
The variant of the CHSH inequality that we use is not used in the self-testing literature befor.
...
Since the correlation we designed satisfies two sub-tests, we introduce one test
in \cref{sec:lsg} and the other one in \cref{sec:chsh}. Our main result is based on the combination
of the sub-tests and presented in \cref{sec:main}. 
...
measurements, which is a phenomenon referred as a self-test. We give formal statement of this self-testing result in \cref{sec:chsh}.
...
The main contribution of our work is the construction of a correlation, which is the optimal correlation of a nonlocal game $\G$ with constant-sized $\calX$,$\calY$,$\calA$ and $\calB$
...
We introduce the definition of self-testing, the definition of linear system game, 
