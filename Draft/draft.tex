...
relation $UOU\ct = O^r$, which is a generalization of a relation in Ref.~\cite{slofstra2017}.
...
\begin{proposition}
\label{prop:orthog}
	Let $H$ be a Hermitian matrix and $\ket{u}$ and $\ket{v}$ be two vectors
	such that
	\begin{align*}
...
combination of the linear equations. Then he shows the difference between exact representations
...
\begin{proof}
	...
	\begin{align*}
	    O_B (U_B\ct)^j \ket{\psi_1} &= \frac{1}{2} (\1 - iM_{\nr+2})(M_{\nr+1}^0 + i M_{\nr+2}M_{\nr+1}^1)  O_B (U_B\ct)^j \ket{\psi}\\
	                                & \appd{O(r^j \se)} \frac{1}{2} (\1 - iM_{\nr+2})(M_{\nr+1}^0 + i M_{\nr+2}M_{\nr+1}^1) (U_B\ct)^j O_B^{r^j} \ket{\psi} \\
    ...
    %\end{align*}
    %In short, we have shown that 
    With similar reasoning, we can also get
    \begin{align}
    \label{eq:A_omega}
        O_A (U_A\ct)^j \ket{\psi_1} \appd{O(\sd r^{j+1/2} \qe)}\omega_d^{-r^j}  (U_A\ct)^j \ket{\psi_1}.
    \end{align}
	Hence for $j \neq j'$, Applying \cref{prop:orthog} to \cref{eq:B_omega} gives us that  
	\begin{align*}
