...
In order to accomplish our goal, we introduce new techniques for robust self-testing.
We use a different variant of the weighted CHSH inequality, \hf{which was first introduced in Ref.~\cite{acin2012}
for studying randomness and entanglement and not used in the self-testing literature before}, 
to enforce the eigenvalue of the product of two binary observables used in the weighted CHSH test.
The idea of decompose the unitaries of arbitrary order into binary observables was first introduced by Slofstra \cite{slofstra2017}.
We use a different way to decompose the unitaries, which can be seen as the inverse of Jordan's lemma, and
apply it to self-testing proofs.
Comparing to the previous approaches, which aim to enforcing eigenvalues of individual operators, our approach
gives us more flexibility in designing the correlation and easier generalization of our proof techniques. 
The other contribution is that we prove self-testing without using the anti-commutation relation between Pauli operators,
or the generalized version of it, 
which is the core idea in \hf{most of} the previous self-testing results.
\carl{Are we sure about that ("all")?}
Instead, we find a new relation that should be enforced. 

pair of operators that can generate the ring of matrices over complex numbers.
