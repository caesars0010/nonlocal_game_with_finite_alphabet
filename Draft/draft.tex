...
Ensuring that every conjugacy relation is associating two commuting operators is the purpose of introducing $\Pg_1$. 
...
\hfc{I have added some explanation about the derivation. How does it look?}
...
a representation $\Psi_1$ of $\Pg_1$, which is defined on
$\C^2 \x \calH$.
...
Consequently, the representations of $a_1, a_2, a_3$ and
$a_4$ are
\begin{align}
...
\carl{[What convention?  Be a little more specific about what papers this convention is used in.]}, which defines
\begin{align*}
	&Z_A := M_1 && X_A := M_2\\
...
Then we explain how they are combined in the full test and present the ideal strategy for the full test in \cref{sec:ideal_strat}. \carl{[I'm a little skeptical that the phrase ``pass the three test'' is meaningful here -- I don't know that these are the kind of tests that can be ``passed.'']}
\hfc{I have removed that phrase. How is my changes?}
The full correlation, denoted by $\fC(r,d)$, is generated by the ideal strategy and presented in \cref{sec:cor}.

%----------------------------
\subsection{The components of the full test}
\label{sec:tests}
%----------------------------
\textbf{The linear system game $\LS(r)$} (\cref{def:lsg}) consists of
\begin{align*}
    & \calX = \{I_i\}_{i=1}^{\mr}, \\
    & \calY = \{ x(s) : s \in S \},
\end{align*}
...
Hence, the answer sets are 
\begin{align*}
    \calA = \Z_2^3 && \calB = \Z_2.
\end{align*}
The probability distribution is uniform over all the pairs $(I_i, x(s))$ such that $s \in I_i$.
...
The ideal strategy of $\LS(r)$ follows the representation $\Psi$ of $\Gamma$.
...
\textbf{The extended weighted CHSH test} consists of
\begin{align*}
    &\calX = \calY = \{0, 1, 2, \nr+1, \nr+2\} \\
    &\calA = \calB = \{0,1,2\}.
\end{align*}
The probability distribution over the question pairs is uniform.
The reason why this test is called the extended weighted CHSH test will be clear after we introduce
the ideal strategy.
...
\begin{equation}
    \label{eq:tpsi}
    \begin{aligned}
        \ket{\tpsi} = & \frac{1}{\sqrt{d-1}} \sum_{j=1}^{d-1} \ket{x_j}\ket{x_{d-j}}\\
         =& \frac{1}{\sqrt{d-1}} \sum_{j=1}^{(d-1)/2} e^{ij\pi/d}(\ket{j}\ket{j} + \ket{d-j}\ket{d-j}).
    \end{aligned}
\end{equation}
Here we introduce the third basis of $W_{d-1}$: $\{ \ket{j} \}_{j=1}^{d-1}$, so that
...
The corresponding projective measurements are $\Pi_V$ and $\Pi_{V^\perp}$. 
Note that $V$ is the subspace on which they should maximize $\I_{\cot(-\pi/d)}$.
...
\end{align*}
Alice's measurements for questions $0, \nr+1$ and $\nr+2$ are
\begin{align*}
	&\tM_0^0 = \Pi_V && \tM_0^2 = \tM_{\nr+1}^2 = \tM_{\nr+2}^2 = \Pi_{V^\perp}\\
	&\tM_{\nr+1}^0= \ketbra{1}{1} && \tM_{\nr+1}^1 = \ketbra{d-1}{d-1}\\
	&\tM_{\nr+2}^0 =  \ketbra{1_{\times}}{1_{\times}} && \tM_{\nr+2}^1 =  \ketbra{(d-1)_{\times}}{(d-1)_{\times}}.
\end{align*}
Bob's measurements for questions $0, \nr+1$ and $\nr+2$ are
\begin{align*}
    &\tN_0^0 = \Pi_V && \tN_0^2 = \tN_{\nr+1}^2 = \tN_{\nr+2}^2 = \Pi_{V^\perp}\\
    &\tN_{\nr+1}^0= \ketbra{1}{1} && \tN_{\nr+1}^1 = \ketbra{d-1}{d-1}\\
	&\tN_{\nr+2}^0=  \ketbra{1_{\times}}{1_{\times}} && \tN_{\nr+2}^1 =  \ketbra{(d-1)_{\times}}{(d-1)_{\times}}.
\end{align*}
Observe that the ideal strategy ensures that when 
...
\end{align*}
What they measure on $V^\perp$ is irrelevant for this test. Later we will see that their measurements on $V^\perp$ are determined by their observables for variables $x(a_1)$ and $x(a_2)$ of $\LS(r)$.
In other words, they never answer $2$ given question $1$ or $2$. 

There are two key observations.
The first one is that when Alice gets $x \in \{1,2\}$ and Bob gets $y \in \{\nr+1, \nr+2\}$, 
their answers $a,b \in \{0,1\}$ should maximize $\I_{\cot(-\pi/d)}$ with Alice and Bob's roles flipped.
The second one is that when Alice gets $x \in \{\nr+1,\nr+2\}$ and Bob gets $y \in \{1, 2\}$,
...
There are $8$ pairs of observables that we test, 
so Bob's input set consists of all the question pairs
\begin{align*}
...
by examining the probability distribution generated by the ideal strategy.
...
%-----------------------------------------------
The question pairs of the full test is the union of the question pairs of each component.
The only modification is that we changed question $1$ and $2$ from the extended weighted CHSH test to
the question $x(a_1)$ and $x(a_2)$ from $\LS(r)$. 
The three components of the full test are conducted with equal probability.

The ideal strategy for the full test is an extension of the ideal strategy for $\LS(r)$ to make it able to produce the ideal correlation of 
the extended weighted CHSH test.
Note that the ideal strategy of the extended weighted CHSH test is defined on $W_{d-1}$ and the ideal strategy of $\LS(r)$ is defined on $W_2^{\x 2} \x W_{d-1}$, 
so we can modify the ideal strategy of the extended weighted CHSH test by tensoring the shared state with $\frac{1}{2} (\ket{x_1}\ket{x_1} + \ket{x_2}\ket{x_2})^{\x 2}$
and tensoring each projective measurement with $\1_{W_2}^{\x 2}$.
The effect is the modified strategy for the extended weighted CHSH test 
becomes compatible with that of $\LS(r)$,
but the generated correlation remains the same.
Consequently, the ideal strategy for the full test is defined on $W_2^{\x 2} \x W_{d-1}$.
The shared state is 
\begin{align*}
    \ket{\tpsi} = \frac{1}{2} (\ket{x_1}\ket{x_1} + \ket{x_2}\ket{x_2})^{\x 2} \x \left(\frac{1}{\sqrt{d-1}} \sum_{j=1}^{d-1} \ket{x_j}\ket{x_{d-j}}\right).
\end{align*}
The observable for each question of $\LS(r)$ is the same as in the ideal
strategy of $\LS(r)$. For each question from the extended weighted CHSH test, it uses projective measurements from the extended strategy of the test. 

To make sure the strategy is correct, we need to check a few properties of it.
First of all, it is easy to see that this ideal strategy can win the commutation games 
because the observables for questions $x(f_0), x(f_2),
x(g_0)$ and $x(g_2)$ act trivially on $W_{d-1}$, which is the subspace the
measurements for questions $\nr+1$ and $\nr+2$ act on.
To make sure it wins $\LS(r)$, what we need to check is summarized in the 
following proposition.
\begin{proposition}
The observables $\tM(a_j) = \tN(a_j) = \Psi(a_j)$ for $1 \leq j \leq 4$, where $\Psi$ is defined in \cref{def:rep_gamma}, 
and the shared state $\ket{\tpsi}$ defined in \cref{eq:tpsi}
satisfy the relations
\begin{align}
	\label{eq:mn_preserve} &\tM(a_j) \tN(a_j) \ket{\tpsi} = \ket{\tpsi},
\end{align}
for $j=1,2,3,4$.
\end{proposition}
\begin{proof}
	...
	Using \cref{eq:o1,eq:o2}, it is straightforward to see that $\tM(a_j)\tN(a_j) \ket{\tpsi} = \ket{\tpsi}$ for $j = 1,2$.
	Before checking the two cases $j=3$ and $j=4$, we first observe that
	\begin{align*}
		\tM(a_3)\tM(a_4) \tN(a_3) \tN(a_4) \ket{\tpsi} = \ket{\tpsi}.
	\end{align*}
	The eigenvectors of $\tM(a_3)\tM(a_4)$ and $\tN(a_3)\tN(a_4)$ are 
    \begin{align*}
        \ket{u_k} = \frac{1}{\sqrt{d-1}} \sum_{j=0}^{d-2} \omega_{d-1}^{jk} \ket{x_{r^j}}.
    \end{align*}
    ...
    \end{align*}
    Then using \cref{eq:u2} we can deduce that $\tM(a_4)$ acts on $\ket{x_{r^j}}$ by
    \begin{align*}
        \tM(a_4) \ket{x_{r^j}} =& \frac{1}{\sqrt{d-1}} \tM(a_4) \left(\ket{u_0} + \omega_{d-1}^{-j(d-1)/2}\ket{u_{(d-1)/2}}  + 
        ...
        =& \ket{x_{r^{d-1-j}}}.
    \end{align*}
    Consequently, we can show that $\tM(a_4)  \tN(a_4) \ket{\tpsi} = \ket{\tpsi}$ because
    \begin{align*}
        \tM(a_4) \tN(a_4) \ket{\tpsi} = \frac{1}{\sqrt{d-1}}  \sum_{j=0}^{d-2} \tM(a_4)  \tN(a_4) \ket{x_{r^j}} \ket{x_{r^{d-1-j}}} 
        = \frac{1}{\sqrt{d-1}} \sum_{j=0}^{d-2} \ket{x_{r^{d-1-j}}} \ket{x_{r^{j}}} =\ket{\psi},
    \end{align*}
	Combining it with the fact that $\tM(a_3)\tM(a_4) \tN(a_3)\tN(a_4) \ket{\tpsi} = \ket{\tpsi}$, we can conclude that
	$\tM(a_3)\tN(a_3) \ket{\tpsi} = \ket{\tpsi}$.
\end{proof}
The similar result, i.e., $\tM(s)\tN(s) \ket{\tpsi} = \ket{\tpsi}$ for $s \in S$ are easy to check because all the other observables are defined by $\tM(a_j), \tN(a_j)$ for $j=1,2,3,4$ and $\tM(f_0), \tM(f_2), \tM(g_0), \tM(g_2)$ and $\tN(f_0), \tN(f_2), \tN(g_0), \tN(g_2)$, which only act non-trivially on $W_2$.
...
\label{sec:imp_lct}
%------------------------------------------------------------------------------------------
In the first half of this subsection, we use $(i,x(s))$ to denote the question pair so that Alice gets equation number $i$ and Bob gets a variable label $x(s)$ for some $s \in S$. For simplicity, we let Bob get the generator $s$ instead. 
...
\begin{proposition}
	...
	\end{align}
\end{proposition}
Recall all the relations in \cref{eq:mg1,eq:mg2} are embedded in constant number of linear relations in $\Gamma$,
...
\begin{align*}
 \bra{\psi} (M_{\nr+1}^0+M_{\nr+1}^1)N_0^0 \ket{\psi} = \frac{2}{d-1} - O(r \ep).
\end{align*}
On the other hand, the correlation also tells us that 
\begin{align*}
	\bra{\psi} N_0^0 \ket{\psi} \appd{O(r\ep)} 
 \bra{\psi}(M_{\nr+1}^0+M_{\nr+1}^1)\ket{\psi} \appd{O(r\ep)} \frac{2}{d-1},
\end{align*}
which implies that 
\begin{align*}
...
%Another conclusion we can draw is that $B_\ast^0 = A_\ast^0$ and $B_\ast^\perp = A_\ast^\perp$.
%We can assume the Schmidt decomposition of $\ket{\psi}$ is
%\begin{align}
%	\ket{\psi} = \sum_{i=1}^d \lambda_i \ket{i}\ket{i}.
%\end{align}
%If $\supp{A_\ast^0} \neq \supp{B_\ast^0}$, then $B_\ast^0\ket{\psi} \neq A_\ast^0\ket{\psi}$. Since $B_\ast^0$ and $A_\ast^0$ are projectors,
%By the orthogonality relations, we can also conclude that $A_\ast^\perp = B_\ast^\perp$.


%Maybe we can first conclude that $\spn(A_0^0+A_0^1) = \spn(A_1^0+A_1^1) = \spn(A_\ast^0)$.
\begin{proposition}
	\label{prop:s1s2}
...
\begin{align*}
	...
	\appd{O(\sr\se)}&  \bra{\psi}M_0^0 M_x^aN_y^b M_0^0\ket{\psi} + \bra{\psi}M_0^0 M_x^aN_y^b M_x^\perp\ket{\psi} \\
	&+\bra{\psi}M_x^\perp M_x^aN_y^b M_0^0\ket{\psi} + \bra{\psi}M_x^\perp M_x^aN_y^b M_x^\perp\ket{\psi}\\
