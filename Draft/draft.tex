...
relation $UOU\ct = O^r$, with the inspiration comes from Slofstra \cite{slofstra2017}.
Unlike previous self-testing proofs, which tries to enforce the anti-commutation relation
between Pauli operators of dimension $2$, $XZ=-ZX$, or the generalized anti-commutation relation 
between Pauli operators of higher dimension, $XZ = \omega_d ZX$, the dimension $d$ is implicit in our
relation. 
The implication is that the eigenvalues of $O$ are powers of $\omega_d$, for
some $d$ whose primitive root is $r$, and $U$ only permutes the eigenvectors of $O$. 
Since winning the linear system game requires that Alice
and Bob share a state $\ket{\psi}$, which satisfies the condition that 
$O_A \x O_B \ket{\psi} = \ket{\psi}$, intuitively, we know that 
$\ket{\psi}$ is spanned by eigenvectors of $O$. 
The other specialty of the linear system game is that it forces Alice and Bob to have binary observables
whose products are the speical unitaries $U$ and $O$, because the answers to the questions of the 
...
to compose the linear system game with another test, which we call the extended weighted CHSH test.
...
At this point, we can use the extended weighted CHSH test to certify that
$O$ has eigenvalue $\omega_d$ and $d$ is prime, then we know $O$ also has eigenvalues $\omega_d^j$ for $j = 1 \dots d-1$.
In the end, we can use the generalized swap isometry \cite{yang2013} to prove our robust self-testing.
Note that our robust self-testing proof follows a different line of arguments from what was originally
...
\end{theorem}
We prove this by constructing the solution group $\Gamma(\LS_r)$ of the linear system game,
and we denote the linear system game by $\LS(r)$.
...
\end{itemize}
\end{definition}
The special properties of the generators of $\Gamma(\LS(r))$ are summarized in the two propositions below.
\begin{proposition}
...
\begin{align*}
	...
	%= X_{W_2} \x ( \ketbra{x_1}{x_1} \x \Psi_0(a_i) + \ketbra{x_2}{x_2} \x \1_{W_{d-1}} )\\
...
\begin{align*}
	...
	%= X_{W_2} \x X_{W_2} \x \Psi_0(a_{i}) \\
	\Psi(k_{l,2}) &= X_{W_2} \x \Psi_1(b_j) \\
	%= X_{W_2} \x (\ketbra{x_1}{x_1} \x \Psi_0(a_j) + \ketbra{x_2}{x_2} \x \1_{W_{d-1}})\\
...
\label{sec:full_test}
%==========================
The full test consists of three subtest: the linear system game $\LS(r)$,
...
From the construction of $\Gamma(\LS(r))$, we know that $| I_i| \leq 3$.
...
For simplicity we omit the $i_x$ part in the alphabet $\calX$, since $\LS(r)$
...
One of the possible question is to assign a value to some variable of $\LS(r)$.
...
%------------------------------------------------------------------------------------------
\subsection{Implication of passing $\LS(r)$}
\label{sec:imp_lct}
%------------------------------------------------------------------------------------------
...
\begin{proposition}
	...
	\begin{align}
	 ...
	 \label{eq:psi1_norm} &\norm{\ket{\psi_i}}^2 \appd{O(r\se)} 1/(d-1) \quad \text{ for } i = 1, 2\\
...
%	In the definition of $X_A$, we omit $M_{\nr+2}^\perp$ too.
	...
	we first show that $M_1M_2 \ket{\psi_2}\appd{O(\sr \qe/\sd)} \omega_d^{-1} \ket{\psi_2}$, then we 
	use the fact that $\ket{\psi_2} \appd{O(\sd \sr\qe)} \ket{\psi_1}$ to draw the conclusion.
