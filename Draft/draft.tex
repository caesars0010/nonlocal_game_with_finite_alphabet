...
\author{Honghao Fu
\footnote{\href{mailto:h7fu@umd.edu}{h7fu@umd.edu}}}

\renewcommand\Affilfont{\itshape\small}


\date{}
\affil{Department of Computer Science, Institute for Advanced Computer Studies, and Joint Center for Quantum \break Information and Computer Science, University of Maryland, College Park, MD 20742, USA
\break
}
...
\begin{abstract}
	...
	Since there are 
	infinitely many prime numbers whose smallest multiplicative generators
	are at most $5$ (M. Murty \textit{The Mathematical Intelligencer} $10.4$ ($1988$)), 
...
% that are independent of the local dimension of the self-tested state.
...
Note that the sizes of the input or output alphabet of $\fC(d,r)$ is independent of $d$. 
...
the game $\LS(r)$ is constructed so that the sizes
of the input alphabets for Alice and Bob are of order $\Theta(r)$,
and the answer sets are of size $\Theta(1)$.
Restricting $r \leq 5$ as in \cref{thm:inf} implies that 
$\LS(r)$ only uses constant-sized input and output alphabets.

The reason that we choose \cref{eq:uor} to be the relation enforced
by $\LS(r)$ is the following.
Winning $\LS(r)$ implies that the winning strategy contains binary observables
...
Then we can deduce that the eigenvalues of $O$ must be
of the form $\omega_d^j = e^{i 2j\pi/d}$ for some $j \neq 0$ and some prime $d$ whose primitive root is $r$. 
We can also deduce that $U$ is a permutation of the eigenbasis of $O$.
Winning this linear system game also implies that Alice and Bob share a state
$\ket{\psi}$ such that $\ket{\psi}$
is spanned by tensor products of states in the eigenbasis of $O$,
and is invariant under the permutation induced by $U$
% \begin{align*}
...
\begin{align*}
    ...
    \ket{EPR}^{\x 2}.
\end{align*}
In this proof, we demonstrate a new way to apply the generalized swap-isometry
...
On each subspace $V_j$, we define quantum states 
\begin{align}
...
% To introduce the projectors of the ideal strategy, we
% define $V_1^\perp := \bigoplus_{j=2}^{(d-1)/2} V_j$
% and denote the corresponding projector by $\Pi_{V_1}^{\perp}$.
...
% Note that $V_1$ is the subspace on which they should maximize $\I_{\cot(-\pi/d)}$. We also need to define 
...
\hline
\hline
\multirow{2}{*}{$y = 1$} & $b=0$ & $\frac{\cos^2(\pi/2d)}{d-1}$ & $\frac{\sin^2(\pi/2d)}{d-1}$ & \small $\pr{2,0}{0,0}$ 
& $\frac{1+\sin(\pi/d)}{2(d-1)}$ & $\frac{1-\sin(\pi/d)}{2(d-1)}$ & \small  $\pr{2,0}{1,0}$ \\
\cline{2-8}
&$b=1$ & $\frac{\sin^2(\pi/2d)}{d-1}$ & $\frac{\cos^2(\pi/2d)}{d-1}$ & $\frac{d-3}{d-1}-\pr{2,0}{0,0}$ 
&  $\frac{1-\sin(\pi/d)}{2(d-1)}$ & $\frac{1+\sin(\pi/d)}{2(d-1)}$ & \small $\frac{d-3}{d-1} - \pr{2,0}{1,0}$  \\
\hline
\multirow{2}{*}{$y = 2$} & $b=0$ & $\frac{\cos^2(\pi/2d)}{d-1}$ & $\frac{\sin^2(\pi/2d)}{d-1}$ & \small $\pr{2,0}{0,1}$ & 
$ \frac{1-\sin(\pi/d)}{2(d-1)}$ & $ \frac{1+\sin(\pi/d)}{2(d-1)}$ & \small $\pr{2, 0}{1,1}$  \\
\cline{2-8}
&$b=1$ & $\frac{\sin^2(\pi/2d)}{d-1}$ & $\frac{\cos^2(\pi/2d)}{d-1}$ & \small $\frac{d-3}{d-1}-\pr{2,0}{0,1}$ &  
$ \frac{1+\sin(\pi/d)}{2(d-1)}$ & $ \frac{1-\sin(\pi/d)}{2(d-1)}$ & \small $\frac{d-3}{d-1}- \pr{2,0}{1,1}$ \\
\hline
\end{tabular}
\end{center}
