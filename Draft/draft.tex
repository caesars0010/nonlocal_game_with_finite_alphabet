...
In the CHSH scenario, Alice and Bob has binary observables $A_x,B_y$ for $x,y = 0,1$ 
...
$u = x_3x_4$ and $x = x_1x_2$ by $U$ and $X$.
The key relation that we will use in the later section is that 
\begin{align}
	(A_3A_4) (A_1A_2)(A_4A_3) \ket{\psi} = (A_1A_2)^2\ket{\psi}, 
\end{align}
where $\ket{\psi}$ is the state shared between Alice and Bob and $A_i$ is 
Alice's observable for $1 \leq i \leq n$.
...
\label{thm:selftest}
	...
	defined in \cref{def:ideal}. \hl{Error term depend on $\alpha$.}
\end{theorem}
...
\label{prop:2d-subspace}
	...
	violation of  $\I_{\cot(-\pi/2d)}$, then $\supp(\Tr_A(\ketbra{\psi}{\psi}))$ is spanned by $\omega_d$-eigenvectors 
	and $\omega_d^{-1}$ -eigenvectors of $\tilde{B}_0\tilde{B}_1$.
\end{proposition}
\begin{proof}[Proof of \cref{prop:2d-subspace}]
	...
	maximal value of $\I_{\cot(-\pi/2d)} $ implies that 
	\begin{align}
...
\end{proof}
Suppose the subspace $V = \spn(\ket{u_0}, \ket{u_1})$, then
the actual eigenbasis of $X = B_0B_1$ that we use is 
\begin{align}
...
\end{align}
The form of $B_1$ is important for the whole embedding argument to work in \cref{sec:main}.
...
In this test, Alice and Bob each gets a question $x$,$y \in \{ 0, 1, \ast\}$ and 
they answer with $a,b \in \{0,1,\diamond,\perp\}$. 
Our self-test result follows the optimal correlation of this test.
...
\label{sec:main}
%=====================================
In this section, we introduce the combined game $\G$ and then
