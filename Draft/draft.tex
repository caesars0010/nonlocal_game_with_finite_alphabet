...
\end{align}
\textcolor{red}{If we express $\ket{j}$'s as $\ket{x_j}$'s, then we can find that 
shared state is $\ket{\psi} = 1/2\sum_{j=0}^{d-2} e^{-ir^j\pi/d} (\ketbra{x_{r^j}}{x_{d-r^j}} + \ketbra{x_{d-r^j}}{x_{r^j}}$.}
...
\textcolor{red}{Here I assume that $\Pi_{V_1} \ket{\psi} = \ket{\psi}$ and $\Pi_{V_{d-1}} \ket{\psi} = \ket{\psi}$,
but it is not true.}
