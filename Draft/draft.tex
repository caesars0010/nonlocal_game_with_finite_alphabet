...
\end{theorem}
The correlation that we use to prove \cref{thm:pr_2} is denoted by $C(\dr{2})$.
We use the superscript $(2)$ to denote the primitive root of $d$.
Note that although the size of $C(\dr{2})$ does not depend on $d$, the optimal correlation does.
This correlation can be easily modified to self-test the maximally entangled state whose local dimension has 
primitive $3$ or $5$.
...
We use some basic number theory concepts in our work. A primitive root of a prime number
...
$\Z_d^\times = \{1,2\dots d-1\}$.
The EPR pair of local dimension $d-1$ for some prime $d$, which has primitive root $2$, 
is denoted by
\begin{align}
...
Through out this paper, any operation on the label of the eigenvector $\ket{i}$ is taken modulo $d$,
...
on Hilbert space $\calH_A$ and $\calH_B$ respectively.
A binary observable is a Hermitian matrix whose eigenvalues are either $+1$ or $-1$.
The weighted CHSH inequality is given by 
\begin{align}
	\label{eq:chsh_op}
	\I_\alpha = \alpha\ip{A_1B_1}+\alpha\ip{A_1B_2} + \ip{A_2B_1} - \ip{A_2B_2}\leq 2\alpha,
\end{align}
where $\ip{A_xB_y}$ is the expectation value of of the observables.
...
measurements, which is the phenomenon of self-test. We give formal statement of this self-test result in \cref{sec:chsh}.
...
The main contribution of our work is the construction of a correlation, which is the optimal correlation of a game $\G$ with constant-sized $\calX$,$\calY$,$\calA$ and $\calB$
such that this correlation can self-test $\ket{\EPR{d-1}}$ for arbitrary $d \in D$. 
The nonlocal game $\G$ is a linear system game with strict enforcements. 
We introduce the definition of self-testing first, the definition of linear system game, 
...
\begin{definition}[Linear system game]
 ...
 $c$ is a length-$n$ vector with entries in $\Z_2$ too. 
...
\label{def:op_sol}
	...
	operators $A_1, A_2, \dots A_n$ on a Hilbert space $\calH$ such that 
	\begin{enumerate}
...
\begin{definition}[Solution group of a linear system game]
	...
	\end{align*}
\end{definition}
For concepts about group presentation, we refer to Sec.~$2$ of Ref.~\cite{slofstra2017}.
...
and this correlation is denoted by $C(\dr{2})$.
The correlation we constructed is the optimal correlation for a game $\G(\dr{2})$, which is the combination of two tests.
...
Slofstra's seminal work~\cite{slofstra2017} draws our attention to the relation $xyx^{-1} = y^2$.
The main component of $\G(\dr{2})$ is a linear system game $\LS$, whose solution group is the embedding of the 
following group
\begin{align}
...
\end{proposition}
The process of embedding $\Pg$ into the solution group of $\LS$ follows the recipe given in the proofs of
...
We give details of this embedding process in \cref{sec:power2}.
...
The product of the generators $x_1$ and $x_2$ from the solution group of $\LS$ plays the role 
of the generator $x$ in $\Pg$ and the product of the generators $x_3$ and $x_4$ plays the role
of the generator $u$ in $\Pg$. As shown in \cref{sec:power2}, $x_1$,$x_2$,$x_3$ and $x_4$ satisfy the relation
\begin{align}
	(x_3x_4)(x_1x_2)(x_4x_3) = (x_1x_2)^2.
\end{align}
Hence the perfect quantum strategy $(\ket{\psi}, \{A_i, B_i\}_{i=1}^n)$ should satisfy 
the key relation that 
\begin{align}
	(A_3A_4) (A_1A_2)(A_4A_3) \ket{\psi} = (A_1A_2)^2\ket{\psi}, 
\end{align}
and similarly on Bob's side.
If we denote the unitary corresponding to the generator $u$ of $\Pg$ by $U$ and the unitary
corresponding to $x$ by $X$, then $U = A_3A_4$, $X= A_1A_2$, and 
$UXU\ct \ket{\psi} = X^2 \ket{\psi}$.
...
\label{sec:main}
%=====================================
In this section, we introduce the correlation $C(\dr{2})$, which is the
ideal correlation of $\G(\dr{2})$, and then
prove that it can self-test the state $\ket{\EPR{d-1}}$.

%-----------------------------------------------------------------
\subsection{The correlation $C(\dr{2})$}
%-----------------------------------------------------------------
...
\end{itemize}
Note that the dimension $\dr{2}$ is defined in the rules of $\CHSH_X$.

\begin{proposition}
	The game $\G(\dr{2})$ can be won perfectly by a quantum strategy.
\end{proposition}
...
\begin{align}
	U = \sum_{i=1}^{d-1} \ketbra{x_{i/2 \pmod{d}}}{x_i}.
\end{align}
...
\begin{align}
	UB_2U\ct = \sum_{i=1}^{d-1} U\ketbra{x_i}{x_{d-i}}U\ct.
\end{align}
...
\end{proof}
Next we are going to prove that the correlation $C(\dr{2})$ that is produced by the 
...
\begin{theorem}
	If a quantum strategy using the shared state $\ket{\psi}$ achieves the ideal 2-party correlation $C(d,2)$ where $d$ is an odd
	prime number with primitive root $2$, then there exist local isometries $\Phi_A$ and $\Phi_B$, and a state $\ket{junk}$ such 
...
By induction, we know $X_A (U_A^\dagger)^i \ket{x_{1}} = \omega_d^{2^i} (U_A^\dagger)^i\ket{x_{1}}$. 
...
\begin{align}
	\Pi_{V_1} =  \sum_{j=1}^N \ketbra{x_{1,j}}{x_{1,j}},
\end{align}
then we can construct 
\begin{align}
 \Pi_{V_i} = \sum_{j=1}^N \ketbra{x_{i,j}}{x_{i,j}},
\end{align}
where $\ket{x_{i,j}} = (U\ct)^{k_i} \ket{x_{1,j}}$ for $2^{k_i} \equiv i \pmod{d}$ and $1 \leq j \leq N$.
...
\begin{align}
	\label{eq:d-1}
	\Pi_{W_j} \tau \Pi_{W_j}  = c_j \1_{W_j} \text{ for } j = 1\dots N.
\end{align}
In the vector form, we know
\begin{align}
	\sum_{j =1}^N \Pi_{W_j} \ket{\psi} =\sum_{i=1}^{d-1} \sum_{j=1}^N c_j \ket{x_{i,j}}\ket{x_{i,j}}. 
\end{align}
...
\norm{ \Pi_{V_1} + \Pi_{V_{d-1}} \ket{\psi}}^2 = \norm{\Pi_{V_1} \ket{\psi}}^2 + \norm{\Pi_{V_{d-1}} \ket{\psi}}^2 = \frac{2}{d-1},
 ...
 \end{align}
 Then we can calculate the norm of $\sum_{j =1}^N \Pi_{W_j} \ket{\psi}$, which is
 \begin{align}
 \norm{\sum_{j =1}^N \Pi_{W_j} \ket{\psi}}^2 = \sum_{i=1}^{d-1} \sum_{j=1}^N \norm{c_j}^2 = 1 = \norm{\ket{\psi}}^2.
 \end{align}
 We can conclude that $\supp{\rho_A} = \cup_{i=1}^{d-1} V_i$ and 
 \begin{align}
 	\ket{\psi} = \sum_{i=1}^{d-1}\sum_{j=1}^N c_j \ket{x_{i,j}}\ket{x_{i,j}}.
 \end{align}
...
\begin{align}
	\ket{\psi'} = \sum_{i=1}^{d-1}\sum_{j=1}^N c_j \ket{x_{i,j}}_A\ket{x_{i,j}}_B \ket{j}_A\ket{j}_B.
\end{align}
...
\begin{align}
	 &\sum_{i=1}^{d-1}\sum_{j=1}^N c_j [(O\ct)^j\ket{x_{i,j}}_A] \x [(O\ct)^j\ket{x_{i,j}}_B] \x \ket{j}_A\ket{j}_B\\
	 = &\sum_{i=1}^{d-1}\sum_{j=1}^N c_j \ket{x_{i,1}}_A\ket{x_{i,1}}_B \ket{j}_A\ket{j}_B\\
	 =  &\frac{1}{\sqrt{d}} \sum_{i=1}^{d-1} \ket{x_{i,1}}_A\ket{x_{i,1}}_B \x \sum_{j=1}^N \sqrt{d}c_j \ket{j}_A\ket{j}_B
\end{align}
