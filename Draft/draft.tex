...
\end{theorem}
The correlation that we use to prove \cref{thm:pr_2} is denoted by $C(\dr{r})$.
...
and this correlation is denoted by $C(\dr{r})$.
...
\begin{proposition}
	\label{prop:embed}
	The group $\Pg_r$ can be embedded into a linear system game $\LS$ over $\Z_2$,
...
of the generator $u$ in $\Pg_r$. As shown in \cref{sec:power2}, \cref{sec:power3} and \cref{sec:power5}, 
...
way to test EPR pairs of certain size than the one proposed in Ref.~\cite{cgs2017}.
