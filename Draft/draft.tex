...
\date{}
\affil{
Joint Center for Quantum Information and Computer Science, \break
Institute for Advanced Computer Studies and Department of Computer Science,\break 
...
%Winning this linear system game also guarantees that Alice and Bob share a state
%$\ket{\psi}$ such that 
% \begin{align*}
%     O \x O \ket{\psi} = \ket{\psi}, && 
%     U \x U \ket{\psi} = \ket{\psi}.
% \end{align*}
%From the equations above, we can deduce that
%$\ket{\psi}$ is spanned by tensor products of states in the eigenbasis of $O$,
...
\Cref{sec:main} is devoted to the robust self-testing proof. We end this paper with conclusions
in \cref{sec:conclu}. 
...
\center
	...
	\node[operator] (op43) at (4, -2) {$U_B$} edge[-] (op23);
    	...
    	node[midway,right] (bracket2) {$\ket{\tpsi}^{\EXT}$}
    	(6.5,-2.5);
        %
        % Background Box
