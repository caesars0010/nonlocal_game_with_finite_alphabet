...
In order to construct a representation of $\Gamma(\LS_r)$, we first construct a representation, $\Psi_{G(x)}$ for the group 
\begin{align}
	G(x) = \ip{ \{x_i\}_{i=1}^{n_r(x)}: x_i^2 = e, x_i x_j x_i = x_k \text{ for all } (i,j,k) \in C_r},
\end{align}
where details of $C_r$ can be found in the appendices. 
Note that the relation $uxu^{-1} = x^2$ is embedded in the conjugation relations in $C_r$.
Building on $\Psi_{G(x)}$, we can construct
the representation $\Psi_{\Gamma(\LS_r)}$ for the solution group $\Gamma(\LS_r)$.

Since Bob gets the same symbols $1$ and $2$ in different sub-tests, 
the observables $A_1 = B_1 = \Psi_{G(x)}(x_1)$ and $A_2 = B_2 = \Psi_{G(x)}(x_2)$ are extended from $B_1|_V$ and $B_2|_V$ as follows
%\begin{align}
%	B_1 = \ketbra{x_0}{x_0} + 
...
%\end{align}
\begin{align}
	A_1 = B_1 = \Psi_{G(x)}(x_1) &= \sum_{k=1}^{(d-1)/2}\left( \omega_d^k\ketbra{x_k}{x_{d-k}} + \omega_d^{-k}\ketbra{x_{d-k}}{x_k}\right)\\
	A_2 = B_2 = \Psi_{G(x)}(x_2) &= \sum_{k=1}^{(d-1)/2}\left(\ketbra{x_{d-k}}{x_k} + \ketbra{x_k}{x_{d-k}}\right).
\end{align}
It can be checked that $X = \Psi_{G(x)}(x_1x_2) = \sum_{i=1}^{d-1} \omega_d^i \ketbra{x_i}{x_i}$ 
The relation $uxu^{-1} = x^r$ and $X$ determine that 
\begin{align}
	U =\Psi_{G(x)}(u) = \Psi_{G(x)}(x_3x_4) =  \sum_{i=1}^{d-1} \ketbra{x_{i/r \pmod{d}}}{x_i}.
\end{align}
...
then give $\Psi_{G(x)}(x_3)$ and $\Psi_{G(x)}(x_4)$.
...
%\end{align}
The decomposition of $U$ into $\Psi_{G(x)}(x_3)$ and $\Psi_{G(x)}(x_4)$ is similar to the decomposition of $X$.
...
\end{align} 
then $\Psi_{G(x)}(x_3)$ and $\Psi_{G(x)}(x_4)$ are  
\begin{align*}
	A_3 = B_3 =\Psi_{G(x)}(x_3) = &\ketbra{u_0}{u_0} +\omega_{d-1}^{(d-1)/2}\ketbra{u_{(d-1)/2}}{u_{(d-1)/2}}\\ + 
	&\sum_{k=1}^{(d-3)/2}\left( \omega_{d-1}^k\ketbra{u_k}{u_{d-1-k}} + \omega_{d-1}^{-k}\ketbra{x_{d-1-k}}{x_k}\right)\\ 
	A_4 = B_4  =\Psi_{G(x)}(x_4) = &\ketbra{u_0}{u_0} +\ketbra{u_{(d-1)/2}}{u_{(d-1)/2}} \\+
...
\begin{align}
	\Psi_{G(x)}(x_5) = \Psi_{G(x)}(x_1)\Psi_{G(x)}(x_2) \Psi_{G(x)}(x_1).
\end{align}

Next, we consider the representation $\Psi_{G(x,y,z,w)}$.
