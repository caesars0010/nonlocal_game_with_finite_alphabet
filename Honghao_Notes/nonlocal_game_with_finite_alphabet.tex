...
for all $j \neq j'$, and $\norm{(U_A\ct)^j(U_B\ct)^j A_\ast^\diamond \ket{\psi}} = \sqrt{2/(d-1)}$, we can conclude that 
\begin{align}
...
because
\begin{align}
	(O_A+O_A\ct)(U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1} = (U_A\ct U_B\ct)^{\log_r (d-1)} (O_A^{\log_r (d-1)} +(O_A\ct)^{\log_r (d-1)}) \ket{\psi_1} \\
