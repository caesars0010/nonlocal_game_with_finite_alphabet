...
\begin{align}
	&\ket{u_1^{(0)}} = \cos(-\pi/2d)\ket{u_1} + \sin(-\pi/2d)\ket{u_{d-1}}\\
	&\ket{u_{d-1}^{(0)}} = \sin(-\pi/2d)\ket{u_1} - \cos(-\pi/2d)\ket{u_{d-1}}\\
	&\ket{u_1^{(1)}} = \cos(-\pi/2d)\ket{u_1} - \sin(-\pi/2d)\ket{u_{d-1}}\\
	&\ket{u_{d-1}^{(1)}} = \sin(-\pi/2d)\ket{u_1} + \cos(-\pi/2d)\ket{u_{d-1}}\\
...
\begin{align}
	\frac{\bra{\psi}B_\ast^\diamond A_1^0B_1^0 B_\ast^\diamond\ket{\psi}}{\bra{\psi} B_\ast^\diamond B_\ast^\diamond \ket{\psi}} = 
	\frac{\cos^2(-\pi/2d)}{2}.
\end{align} 
We can rewrite the other correlations in a similar way and get a new set of correlation which match exactly with the optimal correlation of $\langle \1_{\cot(-\pi/2d)}\rangle$ and use the self-testing argument on $\{A_1^0-A_1^1, B_1^0-B_1^1, A_1^0-A_1^1, B_1^0-B_1^1\}$ and $B_\ast^\diamond \ket{\psi}/\|B_\ast^\diamond \ket{\psi}\|$.
...
\end{align}
 ...
 \begin{align}
 	\bra{\psi} A_1^0 (B_1^0 - B_1^1) \ket{\psi} = \frac{\cos(\pi/d)}{d-1},\\
	\bra{\psi} A_1^0 (B_2^0 - B_2^1) \ket{\psi} = \frac{\cos(\pi/d)}{d-1},
 \end{align}
which implies that 
\begin{align}
	\frac{\bra{\psi} A_1^0 (B_1 + B_2)/(2\cos(-\pi/d)) A_1^0 \ket{\psi}}{\norm{A_1^0 \ket{\psi}}^2} = 1.
\end{align}
Equivalently,
\begin{align}
	\frac{B_1+B_2}{2 \cos(-\pi/d)} A_1^0 \ket{\psi} = A_1^0 \ket{\psi}.
\end{align}
With similar reasoning, we can get
\begin{align}
	\frac{B_1+B_2}{2 \cos(-\pi/d)} A_1^1 \ket{\psi} = -A_1^1 \ket{\psi}.
\end{align}
On the other hand, we know
\begin{align}
	(\1+Z_A)X_B A_\ast^\diamond \ket{\psi} = X_A(\1-Z_B) A_\ast^\diamond \ket{\psi}\\
	X_A(\1+Z_B) A_\ast^\diamond \ket{\psi} = X_B(\1-Z_A) A_\ast^\diamond \ket{\psi},
\end{align}
which implies that 
\begin{align}
	\frac{B_1 -B_2}{2\sin(-\pi/d)} A_1^0 \ket{\psi} =A_2 A_1^1\ket{\psi}\\
	\frac{B_1 -B_2}{2\sin(-\pi/d)} A_1^1 \ket{\psi} = A_2 A_1^0 \ket{\psi}.
\end{align}
Denoting $c = \cos(\pi/d)$ and $s = \sin(\pi/d)$, we can write 
\begin{align}
	B_1A_1^0 \ket{\psi} = cA_1^0\ket{\psi} - sA_2A_1^1\ket{\psi}\\
	B_1A_1^1 \ket{\psi} = -cA_1^1\ket{\psi} - sA_2A_1^0\ket{\psi}\\
	B_2A_1^0 \ket{\psi} = cA_1^0\ket{\psi} + sA_2A_1^1\ket{\psi}\\
	B_2A_1^1 \ket{\psi} = -cA_1^1\ket{\psi} + sA_2A_1^0\ket{\psi}.
\end{align}
Then we can calculate
\begin{align}
	B_1B_2 A_1^0 \ket{\psi} =& (c B_1A_1^0 +sA_2B_1A_1^1)\ket{\psi}\\
	=&(c^2A_1^0- csA_2A_1^1 -csA_2A_1^1 -s^2 A_2^2 A_1^0)\ket{\psi}\\
	=& (\cos(2\pi/d) A_1^0 - \sin(2\pi/d) A_2A_1^1)\ket{\psi}.
\end{align}
and 
\begin{align}
	B_1B_2 A_1^1 \ket{\psi} =& (-c B_1A_1^1 +sA_2B_1A_1^0)\ket{\psi}\\
	=&(c^2A_1^1+ csA_2A_1^0 +csA_2A_1^0 -s^2 A_2^2 A_1^1)\ket{\psi}\\
	=& (\cos(2\pi/d) A_1^1 + \sin(2\pi/d) A_2A_1^0)\ket{\psi},
\end{align}
or equivalently,
\begin{align}
	B_1B_2 A_2A_1^1 \ket{\psi} = \sin(2\pi/d) A_1^0\ket{\psi} + \cos(2\pi/d)A_2A_1^1\ket{\psi}.
\end{align}
Combining the two results above we know
\begin{align}
	\label{eq:omegad}&B_1B_2 (A_1^0 + i A_2A_1^1) \ket{\psi} =\omega_d(A_1^0 + iA_2A_1^1) \ket{\psi}\\
	\label{eq:omegadinv}&B_1B_2 (A_1^1 + i A_2A_1^0) \ket{\psi} = \omega_d^{d-1}(A_1^1 +iA_2A_1^0) \ket{\psi}.
\end{align}
Then It is easy to see that $(B_1B_2)^r (A_1^0 + i A_2A_1^1) \ket{\psi} = \omega_d^r (A_1^0 + i A_2A_1^1) \ket{\psi}$.
On the other hand we can calculate the length of $\frac{1}{\sqrt{2}}(A_1^0 + i A_2A_1^1) \ket{\psi}$,
\begin{align}
	\norm{\frac{1}{\sqrt{2}}(A_1^0 + i A_2A_1^1) \ket{\psi}}^2 =&\frac{1}{2} \bra{\psi}(A_1^0 -i A_1^1A_2)(A_1^0 + i A_2A_1^1) \ket{\psi}\\
	=& \frac{1}{2} \left(\bra{\psi}(A_1^0 + A_1^1 \ket{\psi} + i\bra{\psi}A_1^0 A_2A_1^1 \ket{\psi} - i\bra{\psi}A_1^1A_2A_1^0\ket{\psi}\right).
\end{align}
Since $A_1^0 +A_1^1= A_\ast^\diamond$, if we can prove $\bra{\psi}A_1^0 A_2A_1^1 \ket{\psi} = \bra{\psi}A_1^1A_2A_1^0\ket{\psi}=0$,
then we know $\norm{\frac{1}{\sqrt{2}}(A_1^0 + i A_2A_1^1) \ket{\psi}} = \frac{1}{\sqrt{2}} \norm{A_\ast^\diamond \ket{\psi}}$.
We use $\bra{\psi}A_1^0 A_2A_1^1 \ket{\psi}$ as an example since the way to prove $\bra{\psi}A_1^1 A_2A_1^0 \ket{\psi} =0$ is very similar,
\begin{align}
	\bra{\psi}A_1^0 A_2A_1^1 \ket{\psi} =& \bra{\psi} A_\ast^\diamond(\1 + \frac{B_1+B_2}{2\cos(\pi/d)}) A_2 (\1 - \frac{B_1+B_2}{2\cos(\pi/d)}) A_\ast^\diamond\ket{\psi}\\
	=&\bra{\psi} A_\ast^\diamond (\1 + \frac{B_1+B_2}{2\cos(\pi/d)})(\1 - \frac{B_1+B_2}{2\cos(\pi/d)})A_2  A_\ast^\diamond\ket{\psi} = 0.
\end{align}
With similar reasoning we can get that 
\begin{align}
	\norm{\frac{1}{\sqrt{2}}(A_1^1 + i A_2A_1^0) \ket{\psi}}^2 = \frac{1}{2} \norm{A_\ast^\diamond \ket{\psi}}^2.
\end{align}
Moreover, we can show that $\frac{1}{\sqrt{2}}(A_1^0 + i A_2A_1^1) \ket{\psi}$ is perpendicular to $\frac{1}{\sqrt{2}}(A_1^1 + i A_2A_1^0) \ket{\psi}$
by the following reasoning
\begin{align}
	&\bra{\psi} (A_1^0 - i A_1^1A_2)(A_1^1 + i A_2A_1^0) \ket{\psi} \\
	= &i\bra{\psi}(A_1^0A_2A_1^0 - A_1A_2A_1)\ket{\psi}\\
	=& i\bra{\psi} [(\1 + \frac{B_1+B_2}{2\cos(\pi/d)})^2- (\1 + \frac{B_1+B_2}{2\cos(\pi/d)})^2]A_2 \ket{\psi}\\
	=& \frac{2i}{\cos(\pi/d)} \bra{\psi} (B_1+B_2)A_2 \ket{\psi} = 0,
\end{align}
where we used the fact that 
\begin{align}
	\bra{\psi} B_1A_2 \ket{\psi} = \frac{2 \sin(\pi/d)}{d-1} && \bra{\psi} B_2A_2 \ket{\psi} = -\frac{2 \sin(\pi/d)}{d-1}.
\end{align}
Since $\norm{\frac{1}{\sqrt{2}}(A_1^1 + i A_2A_1^0) \ket{\psi} + \frac{1}{\sqrt{2}}(A_1^0 + iA_2A_1^1)\ket{\psi}} = \norm{A_\ast^\diamond\ket{\psi}}$,
$A_\ast^\diamond\ket{\psi}$ and $\frac{1}{\sqrt{2}}(A_1^1 + i A_2A_1^0) \ket{\psi} + \frac{1}{\sqrt{2}}(A_1^0 + iA_2A_1^1)\ket{\psi}$ are related by
an isometry.

From \cref{eq:omegad} and \cref{eq:omegadinv}, we know
\begin{align}
	&B_1B_2 (A_\ast^\diamond - iA_2)(A_1^0 + iA_2A_1^1) \ket{\psi} = \omega_d  (A_\ast^\diamond - iA_2)(A_1^0 + iA_2A_1^1) \ket{\psi},\\
	&B_1B_2 (A_\ast^\diamond - iA_2)(A_1^1 + iA_2A_1^0) \ket{\psi} = \omega_d^{-1}  (A_\ast^\diamond - iA_2)(A_1^1 + iA_2A_1^0) \ket{\psi},
\end{align}
and we further notice that 
\begin{align}
&\frac{1}{2}  (A_\ast^\diamond - iA_2)(A_1^0 + iA_2A_1^1) + \frac{1}{2} (A_\ast^\diamond - iA_2)(A_1^1 + iA_2A_1^0) \\
=&\frac{1}{2} (A_1^0 + iA_2A_1^1-iA_2A_1^0 +A_1^1 + A_1^1 +iA_2A_1^0 -iA_2A_1^1 +A_1^0)\\
=& A_1^0 + A_1^1,
\end{align}
which implies that 
\begin{align}
\frac{1}{2}  (A_\ast^\diamond - iA_2)(A_1^0 + iA_2A_1^1)\ket{\psi} + \frac{1}{2} (A_\ast^\diamond - iA_2)(A_1^1 + iA_2A_1^0)\ket{\psi} = 
A_\ast^\diamond \ket{\psi}.
\end{align}
We can also check the orthogonality between these two states
\begin{align}
	&\bra{\psi} (A_1^0 -iA_2A_1^1)(A_\ast+iX_A)(A_\ast-iX_A)(A_1^1+iA_2A_1^0)\ket{\psi}\\
	=&\bra{\psi}(A_1^0 -iA_1^1A_2+iA_1^0A_2 +A_1^1)(A_1^1 +iA_2A_1^0 -iA_2A_1^1 +A_1^0)\ket{\psi} \\
	=&\bra{\psi}(i A_1^0A_2A_1^0 +A_1^0 - iA_1^1A_2A_1^1 -A_1^1 -A_1^0 +iA_1^0A_2A_1^0 +A_1^1 -iA_1^1A_2A_1^1\ket{\psi}\\
	=&2i \bra{\psi}(A_1^0A_2A_1^0 - A_1^1A_2A_1^1\ket{\psi} = 0,
\end{align}
where we used result established in previous paragraphs.

At this time, we need to introduce a symetric weighted CHSH test on Bob's side and Bob will have operators
\begin{align}
	B_n^0 = A_1^0 && B_n^1 = A_1^1 \\
	B_{n+1}^0 = A_2^0 && B_{n+1}^1 =A_2^1.
\end{align}
Following \cref{eq:omegad} and \cref{eq:omegadinv}, we know 
\begin{align}
&X_A (B_\ast^\diamond + i B_{n+1})(B_n^0 - iB_{n+1}B_n^1) \ket{\psi} =\omega_d^{-1} (B_\ast^\diamond + i B_{n+1})(B_n^0 - iB_{n+1}B_n^1) \ket{\psi}\\
&X_A (B_\ast^\diamond + i B_{n+1})(B_n^1 - iB_{n+1}B_n^0) \ket{\psi} =\omega_d^(B_\ast^\diamond + i B_{n+1})(B_n^1 - iB_{n+1}B_n^0) \ket{\psi}.
\end{align}
So it remains to show that 
\begin{align}
(B_\ast^\diamond + i B_{n+1})(B_n^0 - iB_{n+1}B_n^1)\ket{\psi} =  (A_\ast^\diamond - iA_2)(A_1^0 + iA_2A_1^1) \ket{\psi},
\end{align}	
or equivalently
\begin{align}
	B_{n+1}(B_n^0-B_n^1) \ket{\psi} = -A_2(A_1^0 - A_1^1) \ket{\psi}.
\end{align}
In order to show it, we need the two sets of $X_{A/B}$ and $Z_{A/B}$ from the two
weighted CHSH tests to match, and also, 
\begin{align}
	X_BZ_B\ket{\psi} = -X_AZ_A \ket{\psi}
\end{align}
from the weighted CHSH inequality.
To show the relation above, recall from the SOS decomposition, we have
\begin{align}
	X_AZ_B\ket{\psi} = -X_BZ_A\ket{\psi},
\end{align}
then
\begin{align}
	X_AZ_B \ket{\psi} = X_AZ_A\ket{\psi} = -X_BZ_A\ket{\psi} = -X_BZ_B\ket{\psi}.
\end{align}
