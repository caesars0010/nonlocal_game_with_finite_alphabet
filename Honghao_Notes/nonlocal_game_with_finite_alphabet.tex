...
For each variable $\{ x_i \}_{i=1}^n$, Alice and Bob has operators $A_i$ and $B_i$ respectively and 
$\J$ is mapped to $\sigma_A(\J)$ in Alice's solution and $\sigma_B(\J)$ in Bob's solution.
...
\begin{align}
	\Tr(\rho_A \Pi_{i:H(l,i) \neq 0} A_i) = \Tr(\rho_A \sigma_A(\J)^{b(l)}).
\end{align}
where $\rho_A =  \Tr_B(\ketbra{\psi}{\psi})$, $l$ is an equation label and $H(l,i)$ is the coefficient of $x_i$ in equation $l$.
For any $\ket{v} \in \supp(\rho_A)$,
we have 
\begin{align}
\Pi_{i:H(l,i) \neq 0} A_i \ket{v} = \sigma_A(\J)^{b(l)} \ket{v}.
\end{align}
...
\end{align}
So when $A_1A_2$ is restricted to $\supp(\rho_A)$, it is similar to $(A_1A_2)^2$. From now on,
by $A_1A_2$ we mean $A_1A_2|_{\supp(\rho_A)}$.
We will come back to the implication of the condition $A_1A_2 \sim (A_1A_2)^2$ later.

Next we look at the implication of winning the $\CHSH_{\sigma_x}$ game optimally.
...
%Steps going forward:
%\begin{itemize}
%	\item Use $\CHSH_{\sigma_x}$ and $\CHSH_{\sigma_z}$ to argue that $(A_1A_2)^d = (A_3A_4)^d = e$;
%	\item \hl{[TODO]} Need to argue $A_1A_2 = X_A$ and $A_3A_4 = Z_A$.
%\end{itemize}
