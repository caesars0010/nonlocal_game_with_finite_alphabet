...
\end{align*}
where we use the facts that $A_\ast^\perp \ket{\psi} = A_{n+1}^\perp \ket{\psi}$ and that 
...
Starting with $A_iB_i \ket{\psi} = \ket{\psi}$ for $i = 1 \dots n$, we know that 
\begin{align}
	A_4A_3 \x B_4B_3 \ket{\psi} = A_4 \x B_4 \ket{\psi} = \ket{\psi},
\end{align}
or equivalently,
\begin{align}
	U_A\ct \x U_B\ct \ket{\psi} = \ket{\psi}.
\end{align}
It means that $U_A\ct U_B\ct$ only permutes the Schmidt basis of $\ket{\psi}$.
Moreover, we know $A_\ast^\diamond$ is projecting onto a subset of the Schmidt basis of $\ket{\psi}$.
By \cref{prop:2d-subspace}, we know that 
\begin{align}
	(O_A + O_A\ct)(U_A\ct U_B\ct)^j A_\ast^\diamond \ket{\psi} = (U_A\ct U_B\ct)^j (O_A^{r^j} + (O_A\ct)^{r^j}) A_\ast^\diamond \ket{\psi}
	= 2\cos( \frac{2r^j \pi}{d}) (U_A\ct U_B\ct)^jA_\ast^\diamond \ket{\psi}.
\end{align}
Hence $(U_A\ct)^j(U_B\ct)^j A_\ast^\diamond \ket{\psi}$ is orthogonal to $(U_A\ct)^{j'}(U_B\ct)^{j'} A_\ast^\diamond \ket{\psi}$
for all $j \neq j'$, and $\norm{(U_A\ct)^j(U_B\ct)^j A_\ast^\diamond \ket{\psi}} = \sqrt{2/(d-1)}$, we can write  
\begin{align}
	\ket{\psi} =& \sum_{j = 0}^{(d-3)/2} (U_A\ct)^j(U_B\ct)^j A_\ast^\diamond \ket{\psi}\\
	=& \sum_{j = 0}^{(d-3)/2} (U_A\ct)^j(U_B\ct)^j (\ket{\psi_1} + \ket{\psi_2}).
\end{align}
Or can we write 
\begin{align}
	\ket{\psi} = 1/2 \sum_{j=0}^{d-2} (U_A\ct U_B\ct)^j A_\ast^\diamond \ket{\psi} = 
	1/2 \sum_{j=0}^{d-2} (U_A\ct U_B\ct)^j(\ket{\psi_1}+\ket{\psi_2})?
\end{align}
...
\textbf{Case 1: Write $\ket{\psi} = \sum_{j = 0}^{(d-3)/2} (U_A\ct)^j(U_B\ct)^j (\ket{\psi_1} + \ket{\psi_2})$.}
If we apply the isometry given in \cref{sec:isometry} to the state $\ket{\psi}$, the state will evolve as following.
\begin{enumerate}
	\item Append control registers $A'$ and $B'$:
	\begin{align}
		\ket{\psi} \ket{0}_{A'}\ket{0}_{B'} = 
		\sum_{j=0}^{(d-3)/2} (U_A\ct U_B\ct)^j (\ket{\psi_1}+ \ket{\psi_2}) \ket{0}_{A'}\ket{0}_{B'};
	\end{align}
	\item $QFT_d$ on the control registers:
	\begin{align}
		\to \frac{1}{d}\sum_{k_1,k_2 = 0}^{d-1} 
		\sum_{j=0}^{(d-3)/2} (U_A\ct U_B\ct)^j (\ket{\psi_1}+ \ket{\psi_2}) \ket{k_1}_{A'}\ket{k_2}_{B'};
	\end{align}
	\item Controlled-$O_A$ and $O_B$ (If the control register is in state $\ket{k}_{A'/B'}$ apply
	$O_{A/B}^k$):
	\begin{align}
		&\to \frac{1}{d}\sum_{k_1,k_2 = 0}^{d-1} \sum_{j=0}^{(d-3)/2} O_A^{k_1}(U_A\ct)^j O_B^{k_2}(U_B\ct)^j
		 (\ket{\psi_1}+\ket{\psi_2}) \ket{k_1}_{A'}\ket{k_2}_{B'}\\
		 &= \frac{1}{d}\sum_{k_1,k_2 = 0}^{d-1} \sum_{j=0}^{(d-3)/2} [\omega_d^{-k_1r^j+k_2r^j} (U_A\ct U_B\ct)^j \ket{\psi_1}
		 + \omega_d^{k_1r^j-k_2r^j} (U_A\ct U_B\ct)^j \ket{\psi_2}]
		 \ket{k_1}_{A'}\ket{k_2}_{B'}
	\end{align}
	\item $QFT_d^{-1}$ on the control registers:
	\begin{align}
		\to &\frac{1}{d^2}\sum_{l_1,l_2 = 0}^{d-1}\sum_{j=0}^{(d-3)/2} \omega_d^{k_1(-r^j-l_1)}\omega_d^{k_2(r^j-l_2)}
		 (U_A\ct U_B\ct)^j \ket{\psi_1} \ket{l_1}_{A'}\ket{l_2}_{B'}\\
		 &+ \frac{1}{d^2}\sum_{l_1,l_2 = 0}^{d-1}\sum_{j=0}^{(d-3)/2}\omega_d^{k_1(r^j-l_1)}\omega_d^{k_2(-r^j-l_2)}
		 (U_A\ct U_B\ct)^j \ket{\psi_2} \ket{l_1}_{A'}\ket{l_2}_{B'}\\
		= &\sum_{j=0}^{(d-3)/2}(U_A\ct U_B\ct)^j  \ket{\psi_1} \ket{-r^j}_{A'}\ket{r^j}_{B'}
		+ \sum_{j=0}^{(d-3)/2} (U_A\ct U_B\ct)^j  \ket{\psi_2} \ket{r^j}_{A'} \ket{-r^j}_{B'}
	\end{align}
	\item Controlled-$U_A$ and $U_B$ 
	(If Alice's control register is in state $\ket{k}_{A'}$, apply $U_A^{\log_r k}$,
	if Bob's control register is in state $\ket{k}_{B'}$, apply $U_B^{\log_r (d-k)}$) :
	\begin{align}
		&\to 
		\sum_{j=0}^{(d-3)/2} (U_A\ct U_B\ct)^{j-\log_r (d-r^j)} \ket{\psi_1}  \ket{-r^j}_{A'}\ket{r^j}_{B'}+
		\sum_{j=0}^{(d-3)/2} U_A^j (U_A\ct)^j U_B^j (U_B\ct)^j  \ket{\psi_2} \ket{r^j}_{A'}\ket{-r^j}_{B'}\\
		&= (U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1} \x \sum_{j=0}^{(d-3)/2} \ket{-r^j}_{A'}\ket{r^j}_{B'}
		 +\ket{\psi_2} \x \sum_{j=0}^{d-2} \ket{r^j}_{A'}\ket{-r^j}_{B'}
	\end{align}
\end{enumerate}
Here we use the following fact.
Since $r$ is the primitive root of $d$, we know
\begin{align}
...
\end{align}
\hl{We can see that at this point we need to show $(U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1} = \ket{\psi_2}$.}

Here are some evidence that this equality may hold:
\begin{align}
	&O_A \ket{\psi_2} = \omega_d\ket{\psi_2}  && O_A (U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1} = \omega_d (U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1}\\
	&O_B \ket{\psi_2} = \omega_d^{d-1} \ket{\psi_2} && O_B (U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1} = \omega_d^{d-1} (U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1}.
\end{align}
Moreover, $(U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1}$ is orthogonal to $(U_A\ct U_B\ct)^j A_\ast^\diamond \ket{\psi}$ for all $j = 1\dots (d-3)/2$,
because
\begin{align}
	&(O_A+O_A\ct)(U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1} \\
	=& (U_A\ct U_B\ct)^{\log_r (d-1)} (O_A^{\log_r (d-1)} +(O_A\ct)^{\log_r (d-1)}) \ket{\psi_1} \\
	=& 2\cos(2\pi/d) (U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1}\\
	&(O_A+O_A\ct)(U_A\ct U_B\ct)^j A_\ast^\diamond \ket{\psi}\\
	 =& (U_A\ct U_B\ct)^j (O_A^{r^j} + (O_A\ct)^{r^j}) A_\ast^\diamond \ket{\psi}\\
	=& \cos( \frac{2r^j \pi}{d}) (U_A\ct U_B\ct)^jA_\ast^\diamond \ket{\psi}.
\end{align}
So we can conclude $(U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1} \in \spn(\supp(A_\ast^\diamond \ket{\psi}))$.
On the other hand, considering the fact that $(U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1}$ is orthogonal to $\ket{\psi_1}$, and $\ket{\psi_2}$ is orthogonal
to $\ket{\psi_1}$, we can say that 
\begin{align}
	(U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1} \in \spn(\supp(\ket{\psi_2})).
\end{align}
We can use \cref{eq:xaxb_trans} to conclude that 
\begin{align}
	A_{n+2}  \frac{B_1 - B_2}{-2\sin(\pi/d)} \ket{\psi_1} = \ket{\psi_2}.
\end{align}
which means that $\ket{\psi_1}$ and $\ket{\psi_2}$ has the same Schmidt rank and
the same set of Schmidt coefficients.
Then $(U_A\ct U_B\ct)^{\log_r (d-1)} \ket{\psi_1}$ has the same Schmidt rank as $\ket{\psi_2}$.

\textbf{Case 1: Write $\ket{\psi} = 1/2\sum_{j=0}^{d-2} (U_A\ct U_B\ct)^j (\ket{\psi_1} + \ket{\psi_2})$.}
Using $\Phi_A\x \Phi_B$ defined in \cref{sec:isometry},
 It is easy to calculate 
\begin{align}
	&\Phi_A\x\Phi_B\left( \sum_{j=0}^{d-2} (U_A\ct U_B\ct)^j \ket{\psi_1}\right) = \ket{\psi_1} \x \sum_{j=0}^{d-2} \ket{-r^j}\ket{r^j},\\
	&\Phi_A\x\Phi_B\left( \sum_{j=0}^{d-2} (U_A\ct U_B\ct)^j \ket{\psi_2}\right) = (U_A\ct U_B\ct)^{\log_r(d-1)}\ket{\psi_2} \x \sum_{j=0}^{d-2} \ket{r^j}\ket{-r^j},
\end{align}
Since $\sum_{j=0}^{d-2} \ket{r^j}\ket{-r^j} = \sum_{j=0}^{d-2}\ket{-r^j}\ket{r^j}$, we can write 
\begin{align}
	\Phi_A\x \Phi_B(\ket{\psi}) = \frac{1}{2}\left(\ket{\psi_1} + (U_A\ct U_B\ct)^{\log_r(d-1)}\ket{\psi_2})\right)
	\x \sum_{j=0}^{d-2} \ket{r^j}\ket{-r^j}.
\end{align}

\textbf{Case 3: Write $\ket{\psi'} = \sum_{j=0}^{d-2} (U_A\ct)^j(U_B\ct)^j \ket{\psi_1}$}.
We show that the isometry given in \cref{sec:isometry} can distill a maximally entangled
