...
\label{prop:2d-subspace}
	...
	then  
	\begin{align}
		[(\tB_0\tB_1)^r + (\tB_1\tB_0)^r] \ket{\tpsi} \appd{O(2^{r-1}\sqrt{\epsilon})} 2\cos((r+1)\mu)\ket{\tpsi},
	\end{align}
	for all $r \geq 1$.
\end{proposition}
\begin{proof}[Proof of \cref{prop:2d-subspace}]
	...
	\begin{align}
		[(\tB_1\tB_2)^j + (\tB_2\tB_1)^j]\ket{\tpsi} \appd{O(2^{j-1}\sqrt{\epsilon})} 2\cos((j+1)\mu) \ket{\tpsi},
	\end{align}
	for all $j =1\dots n-1$.
	When $j=n$, we have
	\begin{align}
		&\norm{ (\tB_1\tB_2+\tB_2\tB_1)[(\tB_1\tB_2)^{n-1} + (\tB_2\tB_1)^{n-1}]\ket{\tpsi} - 2\cos(n\mu)(\tB_1\tB_2+\tB_2\tB_1)\ket{\tpsi}}\\
		\leq& \norm{(\tB_1\tB_2+\tB_2\tB_1)} \norm{ [(\tB_1\tB_2)^{n-1} + (\tB_2\tB_1)^{n-1}]\ket{\tpsi} - 2\cos(n\mu)\ket{\tpsi}}\\
	...
	\begin{align}
		[(\tB_1\tB_2)^{n-2} + (\tB_2\tB_1)^{n-2}] \ket{\tpsi} \appd{O(2^{n-3} \sqrt{\epsilon})} 2\cos((n-1)\mu) \ket{\tpsi}.
	\end{align}
	The derivation so far gives us that
	\begin{align}
		&\qquad[(\tB_1\tB_2)^{n} + (\tB_2\tB_1)^{n}] \ket{\tpsi} \\
		&\appd{O((2^{n-1}+2^{n-3})\sqrt{\epsilon})} 2\cos(n\mu)(\tB_1\tB_2+\tB_2\tB_1)\ket{\tpsi} - 2\cos((n-1)\mu) \ket{\tpsi}\\
		&\appd{O(2\sqrt{\epsilon})} [4\cos(n\mu)\cos(\mu) - 2\cos((n-1)\mu)]\ket{\tpsi}\\
		&= (2\cos((n+1)\mu) + 2\cos((n-1)\mu) - 2\cos((n-1)\mu))\ket{\tpsi}\\
		&=2\cos((n+1)\mu) \ket{\tpsi}.
	\end{align}
...
%-----------------------------------------------
We first built the isometry out of $U_A, X_A$ and $U_B$, $X_B$. 
Later we may build $X_A$ out of $A_{m_r+2}^0$ and $U_A$
and build $X_B$ out of $B_1$, $B_2$, $B_{n_r+1}$ and $U_B$.

In the ideal case, Alice and Bob share state $\ket{\psi} = \frac{1}{d} \sum_{i=1}^{d-1}\ket{ii}$.
They first append control register $\ket{0}$ to their share of the shared state and 
...
It means that the quantum strategy $\{ \{A_{m_r+2}^a, A_{m_r+3}^a\}_{a=0,1},\{B_{n_r+1}^\diamond B_1^b B_{n_r+1}^\diamond, B_{n_r+1}^\diamond B_2^b B_{n_r+1}^\diamond\}_{b=0,1}, A_{m_r+1}^\diamond B_{n_r+1}^\diamond \ket{\psi}\}$ violates the $\I_\alpha$ almost optimally, i.e., $\ip{I_\alpha} = 2\sqrt{1+a^2}-\epsilon$. 
Let us write $B_1|_V = B_{n_r+1}^\diamond (B_1^0 - B_1^1) B_{n_r+1}^\diamond$ and
$B_2|_V = B_{n_r+1}^\diamond (B_2^0 - B_2^1) B_{n_r+1}^\diamond$.
From \cref{prop:2d-subspace}, we know that 
\begin{align}
	[(B_1|_VB_2|_V)^r + (B_2|_VB_1|_V)^r] B_{n_r+1}^\diamond \ket{\psi} \appd{O(2^{r-1}\sqrt{\epsilon})} 
	2\cos(\frac{(r+1)\pi}{d}) B_{n_r+1}^\diamond \ket{\psi}.
\end{align}
\hl{How do we relate this relation to $U_B(X_B+X_B\ct)U_B\ct\ket{\psi} \appd{O(r\sqrt{r\epsilon})} (X_B^r+ (X_B\ct)^r) \ket{\psi}$?
I think $B_1|_VB_2|_V$ can be viewed as $X_B|_V$ by arguing that $X_B$ has a block diagonal structure (i.e. Jordan's lemma decomposition).}

Let's denote $X_B|_V = B_1|_VB_2|_V$.
\hl{Assume} 
\begin{align}
U_B (X_B|_V + X_B|_V\ct) U_B\ct B_{n_r+1}^\diamond \ket{\psi} \appd{O(r\sqrt{r\epsilon})} 
[(X_B|_V)^r + (X_B|_V\ct)^r] B_{n_r+1}^\diamond \ket{\psi}.
\end{align}
Combining it with the relation 
\begin{align}
	[X_B|_V^r + (X_B|_V\ct)^r] B_{n_r+1}^\diamond \ket{\psi} \appd{O(2^{r-1}\sqrt{\epsilon})} 
	2\cos(\frac{(r+1)\pi}{d}) B_{n_r+1}^\diamond \ket{\psi}.
\end{align}
We know that 
\begin{align}
	(X_B|_V + X_B|_V\ct) U_B\ct B_{n_r+1}^\diamond \ket{\psi} \appd{O(2^{r-1}\sqrt{\epsilon})}  
	2\cos(\frac{(r+1)\pi}{d}) U_B\ct B_{n_r+1}^\diamond \ket{\psi}.
\end{align}
Comparing it with the fact that
\begin{align}
(X_B|_V + X_B|_V\ct) B_{n_r+1}^\diamond \ket{\psi} \appd{O(\sqrt{\epsilon})}  
	2\cos(\frac{2\pi}{d}) B_{n_r+1}^\diamond \ket{\psi},
\end{align}
We will use the following proposition to draw some conclusion between 
$B_{n_r+1}^\diamond \ket{\psi}$
and $U_B\ct B_{n_r+1}^\diamond \ket{\psi}$.
\begin{proposition}
\label{prop:orthog}
...
and the fact that $(X_B|_V + X_B|_V\ct)$ is Hermitian, we can conclude that
\begin{align}
\bra{\psi}B_{n_r+1}^\diamond U_B\ct B_{n_r+1}^\diamond \ket{\psi} = O(\frac{2^{r-2}\sqrt{\epsilon}}{\cos(2\pi/d) - \cos((r+1)\pi/d)}).
\end{align}
