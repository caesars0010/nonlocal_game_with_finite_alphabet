...
Recall that from the linear system game, we know that $A_iB_i \ket{\psi} = \ket{\psi}$ for $i = 1 \dots n$.
Then we can deduce that 
\begin{align}
	A_3A_4 \x B_3B_4 \ket{\psi} = A_3 \x B_3 \ket{\psi} = \ket{\psi},
\end{align}
or equivalently,
\begin{align}
	U_A \x U_B \ket{\psi} = \ket{\psi}.
\end{align}
It means that $U_A U_B $ only permutes the Schmidt basis of $\ket{\psi}$.
Similarly, we also have $U_A\ct U_B\ct \ket{\psi} = \ket{\psi}$.
...
\end{enumerate}
\end{proposition}
The intuition behind the proposition is that if we apply the projection $(U_A\ct)A_\ast^\diamond U_A^j$, for some $j$, to the state $\ket{\psi}$,
we will get 
\begin{align}
...
\begin{proof}
	First observe that 
	\begin{align}
		\bra{\psi}A_\ast^\diamond (U_AU_B)^{\log_r j} (U_A\ct U_B\ct)^{\log_r j} A_\ast^\diamond \ket{\psi} = \bra{\psi} A_\ast^\diamond \ket{\psi} = \frac{2}{d-1}.
	\end{align}
	
	Since $U_AO_A U_A\ct = O_A^r$ and $U_AO_A\ct U_A\ct = (O_A\ct)^r$ 
	must hold on all the orthogonal components of $\ket{\psi}$, which includes $A_\ast^\diamond \ket{\psi}$,
	we know that for $1 \leq j < j' \leq (d-1)/2$, we have
	\begin{align}
