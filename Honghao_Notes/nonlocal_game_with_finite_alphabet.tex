...
all the $O(r)$ conjugation relations are used in the derivation of $(x_3x_4)x_1x_2(x_4x_3) = (x_1x_2)^r$.

Suppose the corollary is true for $l = k$, then
\begin{align}
	...
	&\appd{O(r\se)} (A_4A_3)^k(A_1A_2)^{r^k-1} A_4A_3 (A_1A_2)^r \ket{\psi} \\
	& \ldots (r^k \text{ such swaps} ) \ldots \\
	&\appd{O(r\se)} (A_4A_3)^{k+1} (A_1A_2)^{r^{k+1}}\ket{\psi},
\end{align}
where each of the $r^k$ swaps introduces a difference of order $O(r\se)$.
In summary, we have 
\begin{align}
